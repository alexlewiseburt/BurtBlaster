...
class GameScene: SKScene {
    ...
    func animateCollapsingLines(linesToRemove:
        Array<Array <Block>>, fallenBlocks: Array<Array<Block>>, completion:() -> ()) {
        ...
        for (columnIdx, column) in fallenBlocks.enumerate() {
            for (blockIdx, block) in column.enumerate() {
                ...
                let delay = (NSTimeInterval(columnIdx) *
                    0.05) + (NSTimeInterval(blockIdx) * 0.05)
                let duration =
                    NSTimeInterval (((sprite.position.y - newPosition.y) / BlockSize) * 0.1)
        ...
        for rowToRemove in linesToRemove {
            for block in rowToRemove {
                ...
                let randomDuration =
                    NSTimeInterval(arc4random_uniform(2)) + 0.5
